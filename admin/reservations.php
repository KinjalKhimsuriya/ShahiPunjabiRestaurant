...
$result = "";
	...
	$per_page = 20;
		...
		if($reserve->num_rows) {
			...
			while($row = $reserve->fetch_assoc()) {
				...
				$result .=  "<tr>
								...
								<td>$suggestions</td>
								 <td>
            <a href='reservations.php?delete=$reservation_code' onclick='return check();' style='color: red;'>
        ...
        if (isset($_GET['delete'])) {
            $delete_code = mysqli_real_escape_string($con, $_GET['delete']);
            if (!empty($delete_code)) {
                $sql = $con->query("DELETE FROM reservation WHERE reservation_code = '$delete_code'");        
